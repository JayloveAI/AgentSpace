...
async def search_collaborators(request: SearchRequest):
    ...
    try:
        ...
        for agent in candidates:
            ...
            similarity = 1 - cosine(query_vector, agent_vector)
...
async def create_pending_demand(payload: dict):
    ...
    # Generate real semantic vector
    tag_text = " ".join(tags) + " " + description
    demand_vector = await embedding_service.get_embedding(tag_text)
...
async def handle_supply_announcement(agent_id: str, payload: dict):
    ...
    3. Returns matching demands to provider

    Provider will then deliver files directly to seekers.
    """
    ...
    if not supply_vector:
        tag_text = " ".join(tags)
        supply_vector = await embedding_service.get_embedding(tag_text)
