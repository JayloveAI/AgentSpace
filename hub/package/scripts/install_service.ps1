...
# 配置服务依赖：Hub 依赖 frps 服务（如果存在）
$frpsService = Get-Service -Name "frps" -ErrorAction SilentlyContinue
if ($frpsService) {
    & $nssmPath set AgentHub DependOnService frps
    Write-Host "  已配置服务依赖: AgentHub 依赖 frps" -ForegroundColor Green
}
